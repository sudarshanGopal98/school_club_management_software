...
public class Student implements Serializable {
    ...
    public boolean isInducted(){
        ...
        if(meetingCount() >= (MainFile.DM.meetings.size() - 1)  &&  tutorialCount() >= 2) {
            if (isThetaMember && totalPoints >= EventPoints.thetaInduct) {
                return true;
            } else if (!isThetaMember && totalPoints >= EventPoints.maoInduct) {
                return true;
            } else return false;
        }else return false;
    }
